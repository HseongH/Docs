...
export class DesktopStateSynchronizer extends Service {
  ...
  startSync = () => {
    ...
    events?.ui.onTabAction(event => {
      if (event.type === 'open-in-split-view' && event.payload.tabId === appInfo?.viewId) {
        const to = event.payload.view?.path ?? workbench.activeView$.value?.location$.value;
      ...
      if (event.type === 'separate-view' && event.payload.tabId === appInfo?.viewId) {
      ...
      if (event.type === 'activate-view' && event.payload.tabId === appInfo?.viewId) {
