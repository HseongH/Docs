  ...
  return (
    <div className={styles.workspaceListWrapper}>
      <div className={styles.workspaceType}>
        <CloudWorkspaceIcon width={14} height={14} className={styles.workspaceTypeIcon} />
...
const LocalWorkspaces = ({
  ...
  return (
    <div className={styles.workspaceListWrapper}>
      <div className={styles.workspaceType}>
        <LocalWorkspaceIcon width={14} height={14} className={styles.workspaceTypeIcon} />
...
export const AFFiNEWorkspaceList = ({ onEventEnd }: { onEventEnd?: () => void }) => {
  ...
  const cloudWorkspaces = useMemo(
    () => workspaces.filter(({ flavour }) => flavour === WorkspaceFlavour.AFFINE_CLOUD) as WorkspaceMetadata[],
    [workspaces]
  );

  const localWorkspaces = useMemo(
    () => workspaces.filter(({ flavour }) => flavour === WorkspaceFlavour.LOCAL) as WorkspaceMetadata[],
    [workspaces]
  );
  ...
  return (
    <ScrollableContainer className={styles.workspaceListsWrapper} scrollBarClassName={styles.scrollbar}>
      {isAuthenticated ? (
        <div>
          <CloudWorkSpaceList
            ...
            currentWorkspaceId={currentWorkspaceId}
          />
          {localWorkspaces.length > 0 && cloudWorkspaces.length > 0 ? <Divider size="thinner" /> : null}
        </div>
      ) : null}
      <LocalWorkspaces
